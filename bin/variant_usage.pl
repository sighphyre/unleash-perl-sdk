#!/usr/bin/env perl
use strict;
use warnings;

use JSON::PP qw(encode_json);
use FindBin;
use lib "$FindBin::Bin/../lib";
use lib "$FindBin::Bin/../.local/lib/perl5";

use Mojo::IOLoop;
use Srv::SDK;

$| = 1;

my $unleash_url = $ENV{UNLEASH_URL} || 'http://localhost:4242/api';
my $api_key = $ENV{UNLEASH_API_KEY} || 'default:development.unleash-insecure-api-token';
my $toggle_name = $ENV{UNLEASH_TOGGLE_NAME} || 'demo_toggle';
my $eval_interval = exists $ENV{UNLEASH_EVAL_INTERVAL} ? $ENV{UNLEASH_EVAL_INTERVAL} : 1;

my $sdk = Srv::SDK->new(
    fetch_features_interval => 1,
    send_metrics_interval   => 1,
    unleash_url             => $unleash_url,
    api_key                 => $api_key,
);
$sdk->initialize();

Mojo::IOLoop->recurring(
    $eval_interval => sub {
        my $variant = $sdk->get_variant($toggle_name, { userId => 1 });
        my $name = (ref($variant) eq 'HASH' && defined $variant->{name}) ? $variant->{name} : 'unknown';
        my $feature_enabled = (ref($variant) eq 'HASH' && exists $variant->{featureEnabled})
            ? ($variant->{featureEnabled} ? 'true' : 'false')
            : 'false';
        my $enabled = (ref($variant) eq 'HASH' && exists $variant->{enabled})
            ? ($variant->{enabled} ? 'true' : 'false')
            : 'false';
        my $payload = (ref($variant) eq 'HASH' && exists $variant->{payload})
            ? encode_json($variant->{payload})
            : 'null';

        print "get_variant($toggle_name) = $name",
            " featureEnabled=$feature_enabled",
            " enabled=$enabled",
            " payload=$payload\n";
    }
);

Mojo::IOLoop->start;
